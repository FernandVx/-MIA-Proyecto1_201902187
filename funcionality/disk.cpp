...
void Disk::generatedisk(string s, string f, string u, string p) {
    ...
    try {
        ...
        disk.mbr_tamano = i+ sizeof(Structs::MBR);
        ...
        disk.mbr_Partition_1 = Structs::Partition();
        disk.mbr_Partition_2 = Structs::Partition();
        disk.mbr_Partition_3 = Structs::Partition();
        disk.mbr_Partition_4 = Structs::Partition();

        FILE *wrt = fopen(p.c_str(), "w+b");
        if (wrt == NULL) {
            string cmm = "mkdir -p \"" + p + "\"";
            system(cmm.c_str());
            cmm = "rmdir \"" + p + "\"";
            system(cmm.c_str());
            wrt = fopen(p.c_str(), "w+b");
        }
        fwrite("\0", 1, 1, wrt);
        fseek(wrt, i - 1, SEEK_SET);
        fwrite("\0", 1, 1, wrt);
        rewind(wrt);
        fwrite(&disk, sizeof(Structs::MBR), 1, wrt);
        fclose(wrt);
...
void Disk::generatepartition(string s, string u, string p, string t, string f, string n, string a) {
    try {
        ...
        if (ext == 0 && shared.compare(t, "l")) {
            throw runtime_error("no hay partición extendida para almacenar la lógica");
        }
        if (used != 0) {
        ...
        try {
            findby(disk, n, p);
            shared.handler("FDISK", "el nombre ya está en uso ");
            return;
        } catch (exception &e) {
        }
