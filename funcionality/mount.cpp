            ...
            if (mounted[i].path == p) {
                for (int j = 0; j < 26; j++) {
                    if (Mount::mounted[i].mpartitions[j].status == '0') {
                        ...
                        mounted[i].mpartitions[j].letter = alphabet.at(j);
                        strcpy(mounted[i].mpartitions[j].name, n.c_str());
                        string re = to_string(i + 1) + alphabet.at(j);
                        shared.response("MOUNT", "se ha realizado correctamente el mount -id=87" + re);
                        return;
                    }
                }
            }
