...
bool Admin::login(vector<string> context, Mount m) {
    mount = m;
    vector<string> required = {"id", "usuario", "password"};
    string id;
    string usuario;
    string password;
...
bool Admin::login(string u, string p, string id) {
    try {
        string path;
...
vector<string> Admin::getElements(string txt, char c) {
    vector<string> v;
    string line;
    if (c == ',') {
        txt.push_back(',');
...
void Admin::grp(vector<string> context, string action) {
...
void Admin::usr(vector<string> context, string action) {
...
void Admin::mkusr(string usr, string pwd, string grp) {
    try {
        if (!(shared.compare(logged.user, "root"))) {
            throw runtime_error("no se puede realizar la acción sin el usuario root");
        }
